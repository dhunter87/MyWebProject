...
class Interest {
  ...
  static insert(visits, user_id, category_id) {
    var interest_id = helpers.insertRow('INSERT INTO interest (visits, user_id, category_id) VALUES (?, ?, ?)',
    [visits, user_id, category_id])
    return interest_id
  }
  ...
  static findUsersTop3Interests(user_id) {
    var row = helpers.getRows('SELECT * FROM interest WHERE user_id = ? GROUP BY visitsToCategory Order By visitsToCategory DESC', [user_id])
